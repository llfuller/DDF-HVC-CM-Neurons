...
for neuron_directory_index in range(len(directories_list)):
    ...
    for a_filename in os.listdir(neuron_directory):
        ...
        if a_filename != "2014_09_10_0013.abf":#"2014_12_11_0017.abf":
        ...
        frequency = np.linspace(0, sampling_rate/2, len(power_spectrum))
        plt.figure()
        plt.plot(frequency[10:], power_spectrum[10:]/np.max(np.abs(power_spectrum)))
        ...
        for tau in range(2,10):
            for D in range(2,10):
                ...
                for beta in beta_arr:
                    for R in R_arr:
                        if (not math.isclose(beta,1)) or (not math.isclose(R,1e-2)):
                            continue
