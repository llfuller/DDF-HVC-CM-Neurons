...
for a_path in full_paths_list:
    ...
    if len(files_to_evaluate)>0 and a_path[last_slash_location+1:] not in files_to_evaluate:
        continue
    ...
    elif varying_hyperparam.lower() =="d":
        conv_MSE_trials_array = np.zeros((np.shape(D_arr)[0], num_trials, sigma_c_list.shape[0]))  # scalar MSE for each R (row) and trial (column)
    ...
    for sigma_c_index, sigma_c in enumerate(sigma_c_list):
        ...
        if varying_hyperparam.lower() == "r":
            ...
            save_utilities.save_txt_with_makedir(
                np.hstack((np.log10(R_arr)[:, None], np.log10(conv_MSE_trials_array[:, :, sigma_c_index]))),
                directory_to_store_txt_data + "text_data/log10(R)_and_log10(conv_MSE)_" + str(
                    varying_hyperparam) + f"_array_sigma_c={sigma_c}.txt")  # saving dim((num_R, num_trials+1)) first column is R_arr, second col is trial 1, .... last col is trial num_trials
            save_utilities.save_and_or_display_plot(MSE_range_fig, "save",
                                                    directory_to_store_plots + "convolution_MSE_metric/temp_compare/" + title + "_MSE_vs_R_for_sigma=" + str(
                                                        sigma_c) + ".png")
        ...
        if varying_hyperparam.lower() == "tau":
            ...
            ax.set_xlabel("tau (timesteps of size 0.02ms)")
            for i in range(num_trials):
                ax.scatter(tau_arr, np.log10(conv_MSE_trials_array[:,i,sigma_c_index]), s=5)#np.log10(conv_MSE_trials_array[:,i,sigma_c_index]), s=5)
            ax.set_ylabel("log10(MSE after convolution with Gaussian)")
            save_utilities.save_txt_with_makedir(
                np.hstack((tau_arr[:, None], np.log10(conv_MSE_trials_array[:, :, sigma_c_index]))),
                directory_to_store_txt_data + "text_data/tau_and_log10(conv_MSE)_" + str(
                    varying_hyperparam) + f"_array_sigma_c={sigma_c}.txt")  # saving dim((num_R, num_trials+1)) first column is R_arr, second col is trial 1, .... last col is trial num_trials
            save_utilities.save_and_or_display_plot(MSE_range_fig, "save",
                                                    directory_to_store_plots + "convolution_MSE_metric/temp_compare/" + title + "_MSE_vs_tau_for_sigma=" + str(
                                                        sigma_c) + ".png")
