...
for lmn in range(1, 5):
    for ijk in θ[60:71]:
        ...
        fc_tau = 1/ (2 * np.pi * ijk)

        Current_lps = calc_Fourier_power_spectrum(loaded_I_1, [i * (1/50000) for i in range(len(loaded_I_1))])

        for h in range(len(loaded_I_1)):
            if Current_lps[h][1] >= fc_tau:
                fc = h
                break
        ...
        for tau in range(5, 10):
            for D in range(8,12):
                ...
                bias = 100 # should be larger than tau*(D-1) or something like that
                ...
                beta = 10e-2
                R = 10e-8
                title = neuron_name_list[use_neuron_index]+' with 0.02 time step, D = '+str(D)+', Beta = '+str("{:.1e}".format(beta))+', R = '+str("{:.1e}".format(R))+' Train Time = '+str(length)+', Centers = '+str(NoCenters)+', tau = '+str(tau)+', cutoff point ='+str(fc)+', transition band ='+str(b)
                print(R)
