...
# This code is an edit branching off of Python code "User Side Code Separate Train and Test Same Epoch Different"
# on March 30, 2022, because I want to use this file to loop through CM data files, which the old code architecture
# cannot be easily changed to accommodate.
...
directories_list = [#'CM_data/'+str(neuron_name_list[0])+'/',
                    ...
                    'HVC_data_2014/'+str(neuron_name_list[2])+"/"+str(neuron_name_list[0])+'/'
                    ]
...
for neuron_directory_index in range(len(directories_list)):
    ...
    for a_filename in os.listdir(neuron_directory):
        ...
        elif reading_abf_or_txt == "txt":
            if "voltage" in a_filename:
                loaded_VI = np.loadtxt(neuron_directory + a_filename)
                V = np.loadtxt(neuron_directory+a_filename)
                I = np.loadtxt(neuron_directory+a_filename[:-11]+"current.txt")
                TT = 0.02 #this is milliseconds (ask Arij what t should be here)
                num_timesteps = V.shape[0]
                t = num_timesteps*TT #measured in seconds
                V_and_I_arr = np.concatenate((V.magnitude, I.magnitude),axis=1)
                t_arr = np.array([t]).transpose()
        ...
        # plt.ylabel("Normalized Power (1.0 = max from whole spectrum)")
        # plt.xlabel("Frequency (Hz)")
        # plt.savefig(directory_to_store_plots+"Power spectrum of test current (Neuron "+str(neuron_name_list[neuron_directory_index])+'_'+str(a_filename[:-4])+")_last_freq_shown=150.png")
        # plt.show()
        #
        # np.savetxt(directory_to_store_txt_data + "Fourier_analysis/" + str(a_filename[:-4]) + "_Fourier_Spectrum_testing_current-full_data.txt",
