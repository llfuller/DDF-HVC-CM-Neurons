...
from convert_to_time_delay import convert_to_time_delay
...
def calc_moments_of_files_in_directory(file_extension, root_directory = "Data2022-50KhZ/7-7-2022/Lilac 242/Neuron 1/"):
    ...
    do_not_use_dir = "earlier format and min" # list of directories to not use.
    ...
    for a_path in full_paths_list:
        ...
        # screen out the last 3 seconds ([600000:]) of current if it is flat
        loaded_I_ones = np.ones((loaded_I.shape))
        loaded_I_temp = copy.deepcopy(loaded_I)
        loaded_I_ones[loaded_I > 30] = np.nan
        loaded_I_ones[loaded_I < -30] = np.nan
        is_flat = (np.sum(loaded_I_ones[600000:])>0) # boolean, True if step functions after 12 seconds
        if is_flat == True:
            loaded_I_temp[600000:] = np.nan
        loaded_I = loaded_I_temp[np.logical_not(np.isnan(loaded_I_temp))] # remove nans (originally flat) from consideration

        # convert loaded current I to a time-delayed vector
        D_E = 3
        len_skips = 10
        I_td = convert_to_time_delay(arr=loaded_I,
                                       D_E=D_E,
                                       len_skips=len_skips,
                                       num_tsteps_in_time_delay_vector=100000,
                                       timestep_start=D_E * len_skips)

        # calculate moments
        moments = np.zeros(num_moments)
        moments[0] = np.nanmean(loaded_I)
        ...
        if "neuron 2" in a_path.lower():
            if "epoch_10" in a_path.lower():
                print("Neuron 2 epoch 10")
                print("Average: " + str(np.average(loaded_I)))
                plt.figure()
                plt.plot(loaded_I)
                plt.title("Neuron 2 epoch 10")
                plt.show()

        if "I_920061fe" in a_path:
            print("I_920061fe_2014_12_11_0017")
            print("Average: " + str(np.average(loaded_I)))
            plt.figure()
            plt.plot(loaded_I)
            plt.title("I_920061fe_2014_12_11_0017")
            plt.show()
...
def find_nearest_moments(arr_moments_experimental, dict_generated_current_array):
    ...
    arr_max_gen_current_moment = list(np.zeros((num_moments)))
    ...
    for key, value in dict_generated_current_array.items():
        # if "earlier format and min" in key: #skip these files (never sent to Arij)
        #     continue
        generated_name = key
        generated_moments = value
        print("Max is :"+str(arr_max_gen_current_moment))
...
print("experimental_current_dict:")
loaded_npy = np.load("data_derived/current_matching/Data2022-50KhZ/experimental_current_moments.npy", allow_pickle=True)
experimental_current_dict = loaded_npy.item()
print(experimental_current_dict)
