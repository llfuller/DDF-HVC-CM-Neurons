...
class Gauss:    
    ...
    def FuncApproxF(self,Xdata,length,centers,beta,R,D,stim,tau):
        ...
        PhiPhi = np.linalg.inv(np.matmul(PhiMatVolt,PhiMatVolt.T)+beta*np.identity(2*NcLength+1))
        W = np.matmul(YPhi,PhiPhi)
        self.W = W
            
        #Now we want to put together a function to return
        def newfunc(x,stim):
            gaussians = np.exp(-(np.linalg.norm((x-centers),axis=1)**2)*R)
            f = np.matmul(W[0:NcLength],gaussians)
            f = f + W[NcLength]*(stim[0]+stim[1])*0.5
            # Add in extra term
            f = f - np.matmul(W[NcLength+1:],np.multiply(x[0],gaussians))
            return f
    ...
    def CreatePhiMatVoltage(self,X,C,Nlength,NcLength,R,stim,D):
        ...
        Mat = np.zeros((2*NcLength+1,Nlength),dtype = 'float64')
        for i in range(NcLength):
            ...
            Mat[i] = Norm
        # Usual RBF DDF Terms
        gaussians = np.exp(-(Mat[0:NcLength][0:Nlength]**2)*R)
        Mat[0:NcLength][0:Nlength] = gaussians
        Mat[NcLength] = 0.5*(stim[0:Nlength].astype('float64')+stim[1:1+Nlength].astype('float64'))
        # New Voltage RBF DDF Term (Contains V(t) * {Set of Gaussians}
        used_X = X[0:Nlength,0]
        Mat[NcLength+1:][0:Nlength] = - np.multiply(used_X,gaussians)
        return Mat
